export const PONTIX_MESSAGE_VERSION = 1;
export const PONTIX_SELECTION_TTL_MS = 5 * 60 * 1000;

export const PONTIX_SECRET_STORAGE_KEYS = [
    'translatorSecrets',
...
export function extractLocalSecrets(settings = {}) {
    return {
...
export function boundedSelectionSnapshot(payload = {}, sender = {}, now = Date.now()) {
...
export function isSelectionFresh(snapshot, now = Date.now()) {
    return Boolean(snapshot?.expiresAt && snapshot.expiresAt > now);
}

export function validateInternalMessage(message = {}, sender = {}, runtimeId = '') {
    ...
    if (action === 'textSelected') {
        if (!Number.isInteger(sender.tab?.id)) {
            return { ok: false, error: 'missing_tab_context' };
        }
        if (sender.frameId !== undefined && sender.frameId !== 0) {
            return { ok: false, error: 'unsupported_frame_context' };
        }
    }
...
export function shouldAttach2000NlBearer(url) {
    try {

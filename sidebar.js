...
async function fetchGoogleLanguages() {
    ...
    try {
        // Using the public Google Translate languages endpoint
        const response = await fetch('https://translation.googleapis.com/language/translate/v2/languages?key=' + settings.apiKeys.google);
        const data = await response.json();
        
        if (data.data && data.data.languages) {
            const languages = data.data.languages.map(lang => lang.language.toLowerCase());
            console.log('✅ Google languages fetched:', languages.length);
            return languages;
        }
    } catch (error) {
        console.warn('⚠️ Failed to fetch Google languages:', error);
    }
    
    // Fallback to a comprehensive list based on Google's supported languages
...
async function fetchDeepLLanguages() {
    ...
    if (!apiKey) {
        ...
        return ['bg', 'cs', 'da', 'de', 'el', 'en', 'en-gb', 'en-us', 'es', 'et', 'fi', 'fr', 'hu', 'id', 'it', 'ja', 'ko', 'lt', 'lv', 'nb', 'nl', 'pl', 'pt', 'pt-br', 'pt-pt', 'ro', 'ru', 'sk', 'sl', 'sv', 'tr', 'uk', 'zh'];
    }
...
async function fetchMicrosoftLanguages() {
    ...
    // Fallback list
    return ['af', 'ar', 'bg', 'bn', 'bs', 'ca', 'cs', 'cy', 'da', 'de', 'el', 'en', 'es', 'et', 'fa', 'fi', 'fr', 'ga', 'gu', 'he', 'hi', 'hr', 'hu', 'id', 'is', 'it', 'ja', 'kn', 'ko', 'lt', 'lv', 'ml', 'mr', 'ms', 'mt', 'nb', 'nl', 'pa', 'pl', 'pt', 'ro', 'ru', 'sk', 'sl', 'sr', 'sv', 'ta', 'te', 'th', 'tr', 'uk', 'ur', 'vi', 'zh'];
}
...
async function fetchYandexLanguages() {
    ...
    if (!apiKey) {
        ...
        return ['az', 'be', 'bg', 'ca', 'cs', 'da', 'de', 'el', 'en', 'es', 'et', 'fi', 'fr', 'hr', 'hu', 'it', 'lt', 'lv', 'mk', 'nl', 'no', 'pl', 'pt', 'ro', 'ru', 'sk', 'sl', 'sq', 'sr', 'sv', 'tr', 'uk'];
    }
